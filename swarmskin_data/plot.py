...
subject_name_list = ['Taha', 'Luisa', 'Miguel', 'DimaT', 'DimaM', 'Andrey', 'Professor']
# subject_name_list = ['Professor']
...
for name in subject_name_list:
	...
	directory_v = "/home/drone/Desktop/SwarmSkin/experiment1/"+name+"/V/experiment/"
	directory_h = "/home/drone/Desktop/SwarmSkin/experiment1/"+name+"/H/experiment/"
	directory_vh = "/home/drone/Desktop/SwarmSkin/experiment1/"+name+"/VH/experiment/"
	...
	for exper_type in list_of_experiments:
		...
		for experiment_location in experiment_list:
			...
			csv_file_name = experiment_location + '/_slash_vicon_slash_landing_pad_slash_landing_pad.csv'
			with open(csv_file_name) as csvfile:
				reader = csv.reader(csvfile)
				for row in reader:
					if row[10] != "x": # skip the first line
...
print 'len xy_error_list_v', len(xy_error_list_v)
...
# print "\nvariance x_error V total:", np.var(displacement_list_v_x)
# print "variance y_error V total:", np.var(displacement_list_v_y)

# print "variance x_error H total:", np.var(displacement_list_h_x)
# print "variance y_error H total:", np.var(displacement_list_h_y)

# print "variance x_error VH total:", np.var(displacement_list_vh_x)
# print "variance y_error VH total:", np.var(displacement_list_vh_y)
...
# f, ((ax1, ax4), (ax2, ax5), (ax3, ax6)) = plt.subplots(3, 2, sharex='col', sharey='row')
...
# ax1.text(-0.058, 0.05, r'variance x_error V total: ' + ('%.10f' %(np.var(displacement_list_v_x)))[:9]+" m", fontsize=12)
# ax1.text(-0.058, 0.045, r'variance y_error V total:  ' + ('%.10f' %(np.var(displacement_list_v_y)))[:9]+" m", fontsize=12)
# ax2.text(-0.058, 0.055, r'mean xy_error H total:   ' + str(xy_error_list_h.mean())[:6]+" m", fontsize=12)
# ax2.text(-0.058, 0.05, r'variance x_error H total: ' + ('%.10f' %(np.var(displacement_list_h_x)))[:9]+" m", fontsize=12)
# ax2.text(-0.058, 0.045, r'variance y_error H total:  ' + ('%.10f' %(np.var(displacement_list_h_y)))[:9]+" m", fontsize=12)
# ax3.text(-0.058, 0.055, r'mean xy_error VH total:   ' + str(xy_error_list_vh.mean())[:6]+" m", fontsize=12)
# ax3.text(-0.058, 0.05, r'variance x_error VH total: ' + ('%.10f' %(np.var(displacement_list_vh_x)))[:9]+" m", fontsize=12)
# ax3.text(-0.058, 0.045, r'variance y_error VH total:  ' + ('%.10f' %(np.var(displacement_list_vh_y)))[:9]+" m", fontsize=12)
